...
class VOSDataset(VisionDataset):
    ...
    def _get_datapoint(self, idx):
        ...
        for retry in range(MAX_RETRIES):
            try:
                if isinstance(idx, torch.Tensor):
                    idx = idx.item()
                # sample a video
                video, segment_loader, mv_seq, pm_seq = self.video_dataset.get_video(idx)
                ...
                sampled_mvs_and_objs = self.sampler.sample(
                    mv_seq, segment_loader, epoch=self.curr_epoch
                )
        ...
        datapoint = self.construct(video, sampled_frms_and_objs, segment_loader, sampled_mvs_and_objs, pm_seq)
        for transform in self._transforms:
            datapoint = transform(datapoint, epoch=self.curr_epoch)
        return datapoint

    def construct(self, video, sampled_frms_and_objs, segment_loader, sampled_mvs_and_objs, pm_seq):
        """
        Constructs a VideoDatapoint sample to pass to transforms
        """
        sampled_frames = sampled_frms_and_objs.frames
        sampled_mvs = sampled_mvs_and_objs.frames
        sampled_object_ids = sampled_frms_and_objs.object_ids

        images = []
        rgb_images = load_images(sampled_frames)
        # mvs = []
        mv_images = load_images(sampled_mvs)
        blended_images = [
            PILImage.blend(mv_img, rgb_img, alpha=0.3)
            for mv_img, rgb_img in zip(mv_images, rgb_images)
        ]
