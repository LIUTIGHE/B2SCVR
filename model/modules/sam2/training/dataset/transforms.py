...
def hflip(datapoint, index):
    ...
    datapoint.frames[index].mv = F.hflip(datapoint.frames[index].mv)
...
def resize(datapoint, index, size, max_size=None, square=False, v2=False):
    ...
    if v2:
        ...
        datapoint.frames[index].mv = Fv2.resize(
            datapoint.frames[index].mv, size, antialias=True
        )
    else:
        datapoint.frames[index].data = F.resize(datapoint.frames[index].data, size)
        datapoint.frames[index].mv = F.resize(datapoint.frames[index].mv, size)
...
def pad(datapoint, index, padding, v2=False):
    ...
    if len(padding) == 2:
        ...
        datapoint.frames[index].mv = F.pad(
            datapoint.frames[index].mv, (0, 0, padding[0], padding[1])
        )
        h += padding[1]
        w += padding[0]
    else:
    ...
    def __call__(self, datapoint: VideoDatapoint, **kwargs):
        for img in datapoint.frames:
            if self.v2:
                ...
                img.mv = Fv2.to_image_tensor(img.mv)
            else:
                img.data = F.to_tensor(img.data)
                img.mv = F.to_tensor(img.mv)
        return datapoint
    ...
    def __call__(self, datapoint: VideoDatapoint, **kwargs):
        for img in datapoint.frames:
            if self.v2:
                ...
                img.mv = Fv2.convert_image_dtype(img.mv, torch.float32)
                img.mv = Fv2.normalize(img.mv, mean=self.mean, std=self.std)
            else:
                img.data = F.normalize(img.data, mean=self.mean, std=self.std)
                img.mv = F.normalize(img.mv, mean=self.mean, std=self.std)
        ...
        for img in datapoint.frames:
            ...
            for fn_id in fn_idx:
                if fn_id == 0 and brightness_factor is not None:
                    ...
                    img.mv = F.adjust_brightness(img.mv, brightness_factor)
                elif fn_id == 1 and contrast_factor is not None:
                    img.data = F.adjust_contrast(img.data, contrast_factor)
                    img.mv = F.adjust_contrast(img.mv, contrast_factor)
                elif fn_id == 2 and saturation_factor is not None:
                    img.data = F.adjust_saturation(img.data, saturation_factor)
                    img.mv = F.adjust_saturation(img.mv, saturation_factor)
                elif fn_id == 3 and hue_factor is not None:
                    img.data = F.adjust_hue(img.data, hue_factor)
                    img.mv = F.adjust_hue(img.mv, hue_factor)
        return datapoint
...
def random_mosaic_frame(
    ...
    mv_data = datapoint.frames[index].mv
    is_pil = isinstance(image_data, PILImage.Image)
    mv_is_pil = isinstance(mv_data, PILImage.Image)
    if is_pil:
    ...
    if mv_is_pil:
        mv_H_im = mv_data.height
        mv_W_im = mv_data.width
        mv_data_output = PILImage.new("RGB", (mv_W_im, mv_H_im))
    else:
        mv_H_im = mv_data.size(-2)
        mv_W_im = mv_data.size(-1)
        mv_data_output = torch.zeros_like(mv_data)
    ...
    for grid_y in range(grid_h):
        for grid_x in range(grid_w):
            ...
            if (H_im_downsize, W_im_downsize) in downsize_cache:
                ...
                mv_data_downsize = downsize_cache[(H_im_downsize, W_im_downsize)]
            else:
            ...
            if should_hflip[grid_y, grid_x].item():
                ...
                mv_data_downsize = F.hflip(mv_data_downsize)
            ...
            if mv_is_pil:
                mv_data_output.paste(mv_data_downsize, (x_offset_b, y_offset_b))
            else:
                mv_data_output[:, y_offset_b:y_offset_e, x_offset_b:x_offset_e] = (
                    mv_data_downsize
                )

    datapoint.frames[index].data = image_data_output
    datapoint.frames[index].mv = mv_data_output
