...
class SAM2Train(SAM2Base):
    ...
    def forward(self, input: BatchedVideoDatapoint):
        if self.training or not self.forward_backbone_per_frame_for_eval:
            ...
            # print(input.flat_img_batch.shape, input.flat_mv_batch.shape, input.flat_pm_batch.shape) #
    ...
    def forward_tracking(
        ...
        if img_feats_already_computed:
            ...
            (
                ...
                mvs, 
                pms,
            ) = self._prepare_backbone_features(backbone_out, input.mv_batch, input.pm_batch)
        ...
        for stage_id in processing_order:
            ...
            if img_feats_already_computed:
                ...
                # print(len(vision_feats),  vision_feats[0].shape,      len(mvs),   mvs[0].shape,                   len(pms),   pms[0].shape)
                # 3 torch.Size([65536, 10, 32]) 3 torch.Size([3, 10, 1024, 1024]) 3 torch.Size([3, 10])
                current_vision_feats = [x[:, img_ids] for x in vision_feats]
                current_vision_pos_embeds = [x[:, img_ids] for x in vision_pos_embeds]
                current_mv = [x[:, img_ids] for x in mvs]
                current_pm = [x[:, img_ids] for x in pms]
                # print(len(current_vision_feats), len(current_mv), len(current_pm))
                # print(current_vision_feats[0].shape, current_mv[0].shape, current_pm[0].shape)
                # 3 3 3
                # torch.Size([65536, 2, 32]) torch.Size([3, 2, 1024, 1024]) torch.Size([3, 2])
            else:
