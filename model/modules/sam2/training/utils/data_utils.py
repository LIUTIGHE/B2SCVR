...
class BatchedVideoDatapoint:
    ...
    mv_batch: torch.FloatTensor
    pm_batch: torch.FloatTensor
    ...
    @property
    def flat_mv_batch(self) -> torch.FloatTensor:
        """
        Returns a flattened mv_batch_tensor of shape [(B*T)xCxHxW]
        """

        return self.mv_batch.transpose(0, 1).flatten(0, 1)
    
    @property
    def flat_pm_batch(self) -> torch.FloatTensor:
        """
        Returns a flattened pm_batch_tensor of shape 
        """

        return self.pm_batch.transpose(0, 1).flatten(0, 1)
...
class Frame:
    ...
    objects: List[Object]
    mv: Union[torch.Tensor, PILImage.Image]
    pm: str
...
def collate_fn(
    ...
    for video in batch:
        ...
        mv_batch += [torch.stack([frame.mv for frame in video.frames], dim=0)]
        # pm_batch += [torch.stack([frame.pm for frame in video.frames], dim=0)]
    img_batch = torch.stack(img_batch, dim=0).permute((1, 0, 2, 3, 4))
    mv_batch = torch.stack(mv_batch, dim=0).permute((1, 0, 2, 3, 4))  # [T, B, C, H, W]
    
    pm_dict = {'I': 0, 'P': 1, 'B': 2}
    for video in batch:
        pm_batch += [
            torch.tensor([pm_dict[frame.pm] for frame in video.frames], dtype=torch.long)  # 转换为整数索引
        ]
    pm_batch = torch.stack(pm_batch, dim=1)  # [T, B]
    pm_batch = F.one_hot(pm_batch, num_classes=3).to(dtype=img_batch.dtype)
