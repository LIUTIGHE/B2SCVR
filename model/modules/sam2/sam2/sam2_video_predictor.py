...
class SAM2VideoPredictor(SAM2Base):
    ...
    def init_state(
        ...
        with open(pm_path, "r") as f:
            pm_seq = [pm_dict[line.strip()] for line in f.readlines()]
        
        inference_state = {}
        inference_state["images"] = images
        inference_state["mv_maps"] = mv_maps
        inference_state["pms"] = pm_seq
    ...
    def _get_image_feature(self, inference_state, frame_idx, batch_size):
        ...
        image, mv, pm, backbone_out = inference_state["cached_features"].get(
            frame_idx, (None, None, None, None)
        )
        if backbone_out is None:
            # Cache miss -- we will run inference on a single image
            device = inference_state["device"]
            image = inference_state["images"][frame_idx].to(device).float().unsqueeze(0)
            mv = inference_state["mv_maps"][frame_idx].to(device).float().unsqueeze(0)
            pm = torch.tensor(inference_state["pms"][frame_idx], dtype=torch.long)
            pm = F.one_hot(pm, num_classes=3).to(device).float().unsqueeze(0).to(dtype=image.dtype)
            # print(f"loaded sample shape: {image.shape}; loaded mv sample shape: {mv.shape}; loaded pm sample shape: {pm.shape}")
            backbone_out = self.forward_image(image)
            # Cache the most recent frame's feature (for repeated interactions with
            # a frame; we can use an LRU cache for more frames in the future).
            inference_state["cached_features"] = {frame_idx: (image, mv, pm, backbone_out)}

        # expand the features to have the same dimension as the number of objects
        expanded_image = image.expand(batch_size, -1, -1, -1)
        expanded_mv = mv.expand(batch_size, -1, -1, -1)
        expanded_pm = pm.expand(batch_size, -1)
        ...
        features = self._prepare_backbone_features(expanded_backbone_out, expanded_mv, expanded_pm)
    ...
    def _run_memory_encoder(
        ...
        """
        # Retrieve correct image features
        _, _, current_vision_feats, _, feat_sizes, mvs, pms = self._get_image_feature(
