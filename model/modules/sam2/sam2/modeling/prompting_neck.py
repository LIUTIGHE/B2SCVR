...
class ATD_CA(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_tokens=121, reducted_dim=10, qkv_bias=True):
    ...
    def forward(self, x, td, x_size):
        r"""
        Args:
            x: input features with shape of (b, n, c)   torch.Size([2, 4096, 256]) 
            td: token dicitionary with shape of (b, m, c) torch.Size([2, 256, 121])
...
class AC_MSA(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_tokens=121, num_heads=4, category_size=128, qkv_bias=True):
...
class ATDB(nn.Module):
    ...
    def forward(self, x, y, x_size, params):
        return self.patch_embed(self.conv(self.patch_unembed(self.residual_group(x, y, x_size, params), x_size))) + x
...
class PromptingNeck(nn.Module):
    ...
    def forward_features(self, x, y, params):
    ...
    def forward(self, x, y):
        # x: torch.Size([2, 256, 64, 64]), y: torch.Size([2, 256, 121])
        y = y.permute(0, 2, 1)
        # padding
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x, y, params)) + x
            ...
            res = self.conv_after_body(self.forward_features(x_first, y)) + x_first
