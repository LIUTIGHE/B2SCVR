...
class MaskCrossAttention(nn.Module):
    ...
    def forward(self, x, z, m):
        ...
        if self.red > 1:
            ...
            Hr, Wr = H, W
        # for arbitrary variant
        # print(f'reduction: {self.red}, Hr: {Hr}, Wr: {Wr}')
            
        z = F.interpolate(z, size=(x.shape[-2], x.shape[-1]), mode='bilinear', align_corners=False)
