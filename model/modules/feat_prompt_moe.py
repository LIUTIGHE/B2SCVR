...
class Promptor(nn.Module):
    ...
    def __init__(self,
        ...
        if resi_connection == '1conv':
            ...
            self.conv_after_body_1 = nn.Sequential(
                nn.Conv2d(embed_dim, embed_dim // 4, 3, 1, 1), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(embed_dim // 4, embed_dim // 4, 1, 1, 0), nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(embed_dim // 4, embed_dim, 3, 1, 1))
            
        self.clip, _, self.preprocess = create_model_and_transforms('ViT-B-32', pretrained='laion2b_s34b_b79k')
        ...
        # cross attention between GAP-ed image token and CLIP token
    ...
    def forward_features_e0(self, x, params):
        x_size = (x.shape[2], x.shape[3]) 
        # print(x.shape)  # torch.Size([10, 128, 64, 112])
    ...
    def forward_features_e1(self, x, params):
        x_size = (x.shape[2], x.shape[3]) 
        # print(x.shape)  # torch.Size([10, 128, 64, 112])
        x = self.patch_embed(x)
        if self.ape:
    ...
    def forward(self, x, y):
        ...
        '''
        x1 = x.clone()
        x = self.conv_after_body(self.forward_features_e0(x, params))
        # print("x should be:", x.shape) torch.Size([5, 128, 64, 112])
        x1 = self.conv_after_body_1(self.forward_features_e1(x1, params))
        
        # corruption understanding via CLIP
        with torch.no_grad():
            y = F.interpolate(y, size=(224, 224), mode='bilinear', align_corners=False)
            y = self.clip.encode_image(y)
        y = self.clip_mlp(y)
        
        weights = F.softmax(self.gate(y))    
        weights = weights.view(x.shape[0], -1, 1, 1, 1)
        experts = torch.stack([x, x1], dim=1)
        x = torch.sum(weights * experts, dim=1)
        
        # # cross channel attention
        # print(x.shape, y.shape)
        y_cp = self.channel_pooling(y.view(x.shape[0], -1, 1, 1))  
        x_gap = self.global_pooling(x)  # b 128 1 1
        y_cp = y_cp.view(y_cp.size(0), y_cp.size(1), 1, 1) # b 128 1 1
        # print(x_gap.shape, y_cp.shape) torch.Size([5, 128, 1, 1]) torch.Size([5, 128, 1, 1])
        
        # cross-attention-based channel attention
        q = self.wq(y_cp.view(y_cp.size(0), -1))
        k = self.wk(x_gap.view(x_gap.size(0), -1))
        v = self.wv(x_gap.view(x_gap.size(0), -1))
        attn_weight = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(512)
        attn_weight = self.softmax(attn_weight)
        channel_weight = torch.matmul(attn_weight, v)
        channel_weight = self.sigmoid(self.ca_mlp(channel_weight))
        x = x * channel_weight.view(x.size(0), x.size(1), 1, 1)
