...
from model.modules.feat_comp_p import FeatEnhancer
from model.modules.feat_prompt_moe import Promptor
from model.modules.feat_sam_extract import SAM2Extractor
from model.modules.feat_sam_fuse_v3 import SAMFuser
...
class InpaintGenerator(BaseNetwork):
    def __init__(self, init_weights=True):
        ...
        # Hierarchical Augmentation (HA) module via multi-scale SAM feature fusion
        self.sam_fuser = SAMFuser()
        ...
        # Mixture of Residual Experts (MoRE) based prompt module
        self.feat_prompt_module = Promptor(deg_prompt_num=32, embed_dim=128)
    ...
    # def forward(self, masked_frames, corr_content, gt_content, num_local_frames):
    def forward(self, good_frames, corrupted_frames, masks, num_local_frames):
        l_t = num_local_frames
        
        masks_ = masks[:, :l_t, ...].squeeze(0)
        corrupted_frames_ = corrupted_frames[:, :l_t, ...].squeeze(0)
        
        # for arbitrary resolution###########################
        target_size = good_frames.size()[-2:]
        mod = 32
        h_ori, w_ori = target_size
        h_pad = ((h_ori + mod - 1) // mod) * mod - h_ori
        w_pad = ((w_ori + mod - 1) // mod) * mod - w_ori
        h_, w_ = h_ori + h_pad, w_ori + w_pad
        target_size = (h_, w_)
        corrupted_frames_ = F.interpolate(corrupted_frames_, size=target_size, mode='bilinear', align_corners=False)
        masks_ = F.interpolate(masks_, size=target_size, mode='bilinear', align_corners=False)
        ######################################################
        
        sam2_feat = self.sam2_encoder(corrupted_frames_)
        
        # prepare input
        if torch.sum(masks) == 0:
            masks[:, 0, ...] = 1
        masked_frames = (good_frames * (1 - masks).float())  
        corr_content = (corrupted_frames * masks.float())

        b, t, ori_c, ori_h, ori_w = masked_frames.size()    # input size
        ...
        ref_feat = enc_feat.view(b, t, c, h, w)[:, l_t:, ...]   # feature split
        
        local_corr_feat = self.sam_fuser(local_corr_feat.view(-1, c, h, w), sam2_feat, masks_)
        local_corr_feat = local_corr_feat.view(b, -1, c, h, w)
        
        # feature completion
        local_feat = self.feat_enhance_module(local_feat, local_corr_feat)    # for v1p 
        
        # latent restoration    
        local_feat = self.feat_prompt_module(local_feat.view(-1, c, h, w), 
                                             corr_content[:, :l_t, ...].view(-1, ori_c, ori_h, ori_w))
        local_feat = local_feat.view(b, -1, c, h, w)

        # flow completion and feature propagation
        masked_local_frames = (masked_frames[:, :l_t, ...] + 1) / 2 # normalization
